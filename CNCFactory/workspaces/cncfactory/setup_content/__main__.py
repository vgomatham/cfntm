...
def parse_args():
    parser = argparse.ArgumentParser( description='Imports the content into a specified workspace.')
    ...
    parser.add_argument('--workspace-id', required=False, default='CNCFactory', help='workspace to be populated')
    parser.add_argument('--content-root', required=False, default='..', help='content directory relative to this module')

    parser.add_argument('--telemetry-database-name', required=False, default='Telemetry', help='timestream telemetry database name')
    parser.add_argument('--telemetry-table-name', required=False, default='Telemetry', help='timestream telemetry table name')
    parser.add_argument('--telemetry-stack-name', required=False, default=None, help='Name of the Cloudformation stack for resources (like Timestream and Lambda) were created')
...
def main():
    ...
    if args.delete_models or args.delete_all:
        ...
        ws.delete_resource(destination='cnc_mm.glb')
        ws.delete_resource(destination='cnc_lathe.glb')
        ws.delete_resource(destination='CNCFactoryEnvironment.glb')
    ...
    if args.import_component_types or args.import_all:
        ...
        ws.import_component_type(content_path('component_types/cnc_mm_component_type.json'))
        ws.import_component_type(content_path('component_types/cbc_lathe_component_type.json'))
    ...
    if args.import_scenes or args.import_all:
        ...
        ws.import_scene(file_name=content_path('scenes/CNCFactory.json'), scene_name='CNCFactory')
        ws.import_scene(file_name=content_path('scenes/Mixer.json'), scene_name='Mixer')

    # Import models
    if args.import_models or args.import_all:
        print('Importing models...')
        ws.import_resource(file_name=content_path('scenes/CNCFactoryEnvironment.glb'), destination='CNCFactoryEnvironment.glb')
        ws.import_resource(file_name=content_path('scenes/cnc_mm.glb'), destination='cnc_mm.glb')
        ws.import_resource(file_name=content_path('scenes/cnc_lathe.glb'), destination='cnc_lathe.glb')

    # Import telemetry data
    if args.import_telemetry or args.import_all:
        print('Importing sample telemetry data...')
        telemetry.recreate_table()
        telemetry.import_csv(content_path('sample_data/telemetry/cnc_telemetry.csv'), rebase_time_ms=content_start_time_ms)
